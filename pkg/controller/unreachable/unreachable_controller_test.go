...
func TestReconcile(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			errorConnecting:              ptr.To(true),
			errorConnectingSecondary:     ptr.To(false),
			expectedUnreachableStatus:    corev1.ConditionFalse,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.errorConnecting != nil {
				...
				mockRemoteClientBuilder.EXPECT().BuildWithContext(gomock.Any()).Return(nil, buildError)
			}
