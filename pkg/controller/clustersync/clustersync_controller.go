...
const (
	...
	stsName                = hivev1.DeploymentNameClustersync
)
...
func Add(mgr manager.Manager) error {
	...
	r.ordinalID, err = controllerutils.GetMyOrdinalID(ControllerName, logger)
	if err != nil {
		return err
	}
...
func AddToManager(mgr manager.Manager, r *ReconcileClusterSync, concurrentReconciles int, rateLimiter workqueue.TypedRateLimiter[reconcile.Request]) error {
	...
	if err := c.Watch(
		source.Kind(mgr.GetCache(), &hivev1.SyncSet{}, handler.TypedEnqueueRequestsFromMapFunc(requestsForSyncSet))); err != nil {
		return err
	}

	// Watch for changes to SelectorSyncSets
	if err := c.Watch(
		source.Kind(mgr.GetCache(), &hivev1.SelectorSyncSet{}, handler.TypedEnqueueRequestsFromMapFunc(requestsForSelectorSyncSet(r.Client, r.logger)))); err != nil {
		return err
	}
	...
	if err := c.Watch(source.Kind(mgr.GetCache(), &hiveintv1alpha1.ClusterSync{}, &handler.TypedEnqueueRequestForObject[*hiveintv1alpha1.ClusterSync]{})); err != nil {
		return err
	}
	...
	if err := c.Watch(source.Kind(mgr.GetCache(), &hiveintv1alpha1.ClusterSyncLease{}, &handler.TypedEnqueueRequestForObject[*hiveintv1alpha1.ClusterSyncLease]{})); err != nil {
		return err
	}
...
func requestsForSyncSet(ctx context.Context, ss *hivev1.SyncSet) []reconcile.Request {
...
func (r *ReconcileClusterSync) Reconcile(ctx context.Context, request reconcile.Request) (reconcile.Result, error) {
	...
	me, err := controllerutils.IsUIDAssignedToMe(r.Client, stsName, cd.UID, r.ordinalID, logger)
	if !me || err != nil {
		if err != nil {
		...
		logger.Debug("not syncing because this cluster is not assigned to me")
	...
	clusterSync.Status.ControlledByReplica = &r.ordinalID
...
func (r *ReconcileClusterSync) applySyncSet(
	...
	for i, resource := range resources {
		returnErr, requeue = r.applyResource(ctx, i, resource, referencesToResources[i], resourceHelper, applyFnMetricsLabel, logger)
		if returnErr != nil {
	...
	for i, secretMapping := range syncSet.GetSpec().Secrets {
		returnErr, requeue = r.applySecret(ctx, syncSet, i, secretMapping, referencesToSecrets[i], resourceHelper, applyFnMetricsLabel, logger)
		if returnErr != nil {
	...
	for i, patch := range syncSet.GetSpec().Patches {
		...
		returnErr, requeue = r.applyPatch(ctx, i, patch, resourceHelper, logger)
		if returnErr != nil {
			return
		}
	}
...
// Functions using Server-Side Apply with structured results
...
func (r *ReconcileClusterSync) applyResource(
	...
	if err := applyToTargetCluster(ctx, resource, applyFnMetricsLabel, resourceHelper, logger); err != nil {
...
func (r *ReconcileClusterSync) applySecret(
	...
	if err := applyToTargetCluster(ctx, targetSecret, applyFnMetricsLabel, resourceHelper, logger); err != nil {
